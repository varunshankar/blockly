...
function getConfigPorts(actorName) {
	...
	if (container) {
		var blocks = Blockly.Workspace.getByContainer("bricklyDiv")
				.getAllBlocks();
	...
	if (ports.length === 0) {
		ports.push([
				Blockly.Msg.CONFIGURATION_NO_PORT
						|| Blockly.checkMsgKey('CONFIGURATION_NO_PORT'),
				(Blockly.Msg.CONFIGURATION_NO_PORT || Blockly
						.checkMsgKey('CONFIGURATION_NO_PORT')).toUpperCase() ]);
	}
...
Blockly.Blocks['robActions_setLanguage'] = {
	...
	init : function() {
		...
		this.appendDummyInput().appendField(
				Blockly.Msg.SET + ' ' + Blockly.Msg.LANGUAGE).appendField(
				dropdown, 'LANGUAGE');
...
Blockly.Blocks['robActions_sayText_parameters'] = {
	...
	init : function() {
		this.setColour(Blockly.CAT_ACTION_RGB);
		this.appendValueInput('OUT').appendField(Blockly.Msg.SAY);
		this.appendValueInput('VOICESPEED').setAlign(Blockly.ALIGN_RIGHT)
				.appendField(Blockly.Msg.VOICE_SPEED).setCheck('Number');
		this.appendValueInput('VOICEPITCH').setAlign(Blockly.ALIGN_RIGHT)
				.appendField(Blockly.Msg.VOICE_PITCH).setCheck('Number');
		this.setPreviousStatement(true);
		this.setNextStatement(true);
...
Blockly.Blocks['robActions_motor_on'] = {
	...
	init : function() {
		...
		case 'ev3':
			ports = [ [ Blockly.Msg.MOTOR_PORT + ' A', 'A' ],
					[ Blockly.Msg.MOTOR_PORT + ' B', 'B' ],
		...
		case 'botnroll':
			ports = [
					[ Blockly.Msg.MOTOR + ' ' + Blockly.Msg.MOTOR_LEFT, 'B' ],
					[ Blockly.Msg.MOTOR + ' ' + Blockly.Msg.MOTOR_RIGHT, 'C' ] ];
			break;
		case 'mbot':
			ports = [ [ Blockly.Msg.MOTOR + ' ' + 'M1', '1' ],
					[ Blockly.Msg.MOTOR + ' ' + 'M2', '2' ] ];
			break;
		case 'wedo':
			this.action = 'MOTOR';
			ports = [];
			...
			if (ports.length === 0) {
				ports
						.push([
								Blockly.Msg.CONFIGURATION_NO_PORT
										|| Blockly
												.checkMsgKey('CONFIGURATION_NO_PORT'),
								(Blockly.Msg.CONFIGURATION_NO_PORT || Blockly
										.checkMsgKey('CONFIGURATION_NO_PORT'))
										.toUpperCase() ]);
			}
		...
		if (this.workspace.device !== 'wedo') {
			this.appendValueInput('POWER').appendField(dropDownPorts,
					'MOTORPORT').appendField(Blockly.Msg.ON).appendField(
					Blockly.Msg.MOTOR_SPEED).setCheck('Number');
		} else {
			this.appendValueInput('POWER')
					.appendField(Blockly.Msg.ACTION_MOTOR).appendField(
							dropDownPorts, 'MOTORPORT').appendField(
							Blockly.Msg.ON)
...
Blockly.Blocks['robActions_motor_on_for'] = {
	...
	init : function() {
		this.setColour(Blockly.CAT_ACTION_RGB);
		var ports = [ [ Blockly.Msg.MOTOR_PORT + ' A', 'A' ],
				[ Blockly.Msg.MOTOR_PORT + ' B', 'B' ],
				[ Blockly.Msg.MOTOR_PORT + ' C', 'C' ] ];
		...
		if (this.workspace.device === 'wedo') {
			...
			this.appendValueInput('POWER')
					.appendField(Blockly.Msg.ACTION_MOTOR).appendField(ports,
							'MOTORPORT').appendField(Blockly.Msg.ON)
					.appendField(Blockly.Msg.MOTOR_SPEED).setCheck('Number');
			this.appendValueInput('VALUE').setAlign(Blockly.ALIGN_RIGHT)
					.appendField(
							Blockly.Msg.SENSOR_TIME + ' '
									+ Blockly.Msg.SENSOR_UNIT_MS).setCheck(
							'Number');
			...
			if (this.workspace.device === 'arduino') {
				...
				this.appendValueInput('POWER').appendField(
						Blockly.Msg.MOTOR_PORT_ARDUINO).appendField(motorPort,
						'MOTORPORT').appendField(Blockly.Msg.ON).appendField(
						Blockly.Msg.ROTATIONS_PER_MINUTE).setCheck('Number');
			} else {
				this.appendValueInput('POWER').appendField(motorPort,
						'MOTORPORT').appendField(Blockly.Msg.ON).appendField(
						Blockly.Msg.MOTOR_SPEED).setCheck('Number');
			}
			var motorRotation = new Blockly.FieldDropdown([
					[ Blockly.Msg.MOTOR_ROTATION, 'ROTATIONS' ],
			...
			if (this.workspace.device === 'botnroll'
				...
				this.appendValueInput('VALUE').setAlign(Blockly.ALIGN_RIGHT)
						.appendField(Blockly.Msg.FOR).appendField(
								motorRotation, 'MOTORROTATION').setCheck(
								'Number');
			}
			if (this.workspace.device != 'arduino') {
...
Blockly.Blocks['robActions_motor_on_for_ardu'] = {
	...
	init : function() {
		...
		if (this.workspace.device == 'arduino') {
			...
			this.appendValueInput('POWER').appendField(
					Blockly.Msg.ACTION_SERVO_ARDUINO + " " + Blockly.Msg.SET)
					.appendField(dropDownPorts, 'MOTORPORT').appendField(
							Blockly.Msg.TO + ' °').setCheck('Number');
		} else {
			var ports = [ [ Blockly.Msg.MOTOR_PAN, 'A' ],
					[ Blockly.Msg.MOTOR_TILT, 'D' ] ];
			var motorPort = new Blockly.FieldDropdown(ports);
			this.appendValueInput('POWER').appendField(Blockly.Msg.MOTOR)
					.appendField(motorPort, 'MOTORPORT').appendField(
							Blockly.Msg.SET + ' °').setCheck('Number');
		}
		this.setPreviousStatement(true);
		this.setNextStatement(true);
...
Blockly.Blocks['robActions_motor_getPower'] = {
	...
	init : function() {
		...
		var motorPort = new Blockly.FieldDropdown(ports);
		this.appendDummyInput().appendField(
				Blockly.Msg.GET + ' ' + Blockly.Msg.MOTOR_SPEED).appendField(
				motorPort, 'MOTORPORT');
...
Blockly.Blocks['robActions_motor_setPower'] = {
	...
	init : function() {
		...
		var motorPort = new Blockly.FieldDropdown(ports);
		this.appendValueInput('POWER').appendField(Blockly.Msg.SET)
				.appendField(motorPort, 'MOTORPORT').appendField(
						Blockly.Msg.MOTOR_SPEED);
		this.setPreviousStatement(true);
		this.setNextStatement(true);
...
Blockly.Blocks['robActions_motor_stop'] = {
	...
	init : function() {
		...
		if (this.workspace.device === 'mbot') {
			ports = [ [ Blockly.Msg.MOTOR_PORT + ' M1', '1' ],
					[ Blockly.Msg.MOTOR_PORT + ' M2', '2' ] ];
		}
		var motorPort = new Blockly.FieldDropdown(ports);
		if (this.workspace.device === 'wedo') {
			this.action = 'MOTOR';
			...
			var mode = new Blockly.FieldDropdown([
					...
					[ Blockly.Msg.MOTOR_BRAKE, 'NONFLOAT' ] ]);
			this.appendDummyInput().appendField(Blockly.Msg.MOTOR_STOP)
					.appendField(motorPort, 'MOTORPORT').appendField(mode,
							'MODE');
		} else {
			this.appendDummyInput().appendField(Blockly.Msg.MOTOR_STOP)
...
Blockly.Blocks['robActions_motorDiff_on'] = {
	init : function() {
		...
		this.appendValueInput('POWER').appendField(Blockly.Msg.MOTOR_DRIVE)
				.appendField(dropdown, 'DIRECTION').appendField(
						Blockly.Msg.MOTOR_SPEED).setCheck('Number');
		this.setPreviousStatement(true);
		this.setNextStatement(true);
	...
	init : function() {
		...
		if (this.workspace.device === 'botnroll'
			...
			this.appendValueInput('DISTANCE').setAlign(Blockly.ALIGN_RIGHT)
					.appendField(Blockly.Msg.SENSOR_TIME + ' ms').setCheck(
							'Number');
		} else {
			this.appendValueInput('DISTANCE').setAlign(Blockly.ALIGN_RIGHT)
					.appendField(Blockly.Msg.MOTOR_DISTANCE).setCheck('Number');
		}
		this.setPreviousStatement(true);
		this.setNextStatement(true);
...
Blockly.Blocks['robActions_motorDiff_turn'] = {
	init : function() {
		...
		this.appendValueInput('POWER').appendField(Blockly.Msg.MOTOR_TURN)
				.appendField(dropdown, 'DIRECTION').appendField(
						Blockly.Msg.MOTOR_SPEED).setCheck('Number');
		this.setPreviousStatement(true);
		this.setNextStatement(true);
...
Blockly.Blocks['robActions_motorDiff_turn_for'] = {
	init : function() {
		...
		if (this.workspace.device === 'botnroll'
				|| this.workspace.device === 'mbot') {
			this.appendValueInput('DEGREE').setAlign(Blockly.ALIGN_RIGHT)
					.appendField(Blockly.Msg.SENSOR_TIME + ' ms').setCheck(
							'Number');
		} else {
			this.appendValueInput('DEGREE').setAlign(Blockly.ALIGN_RIGHT)
					.appendField(Blockly.Msg.MOTOR_DEGREE).setCheck('Number');
		}
		this.setPreviousStatement(true);
		this.setNextStatement(true);
...
Blockly.Blocks['robActions_motorDiff_curve'] = {
	init : function() {
		...
		this.appendValueInput('POWER_LEFT')
				.appendField(Blockly.Msg.MOTOR_STEER).appendField(dropdown,
						'DIRECTION').appendField(Blockly.Msg.MOTOR_SPEED)
				.appendField(Blockly.Msg.MOTOR_LEFT).setCheck('Number');
		this.appendValueInput('POWER_RIGHT').setAlign(Blockly.ALIGN_RIGHT)
				.appendField(Blockly.Msg.MOTOR_SPEED).appendField(
						Blockly.Msg.MOTOR_RIGHT).setCheck('Number');
		this.setPreviousStatement(true);
		this.setNextStatement(true);
		this.setTooltip(Blockly.Msg.MOTORDIFF_ON_TOOLTIP);
	}
...
Blockly.Blocks['robActions_display_picture'] = {
	...
	init : function() {
		...
		this.appendValueInput('X').setCheck('Number').setAlign(
				Blockly.ALIGN_RIGHT).appendField(Blockly.Msg.X);
		this.appendValueInput('Y').setCheck('Number').setAlign(
				Blockly.ALIGN_RIGHT).appendField(Blockly.Msg.Y);
		this.setPreviousStatement(true);
		this.setNextStatement(true);
...
Blockly.Blocks['robActions_display_text'] = {
	...
	init : function() {
		...
		if (this.workspace.device == 'arduino') {
			...
			this.appendDummyInput().appendField(Blockly.Msg.ACTION_LCD,
					'ACTORTITEL').appendField(dropDownPorts, 'ACTORPORT');
		}
		if (this.workspace.device === 'nxt') {
			this.appendValueInput('OUT').appendField(
					Blockly.Msg.DISPLAY_SHOW + ' ' + Blockly.Msg.DISPLAY_TEXT)
					.setCheck(
							[ 'Number', 'Boolean', 'String', 'Colour',
									'Connection' ]);
		} else {
			this.appendValueInput('OUT').setAlign(Blockly.ALIGN_RIGHT)
					.appendField(
							Blockly.Msg.DISPLAY_SHOW + ' '
									+ Blockly.Msg.DISPLAY_TEXT);
		}
		if (this.workspace.device !== 'botnroll'
...
Blockly.Blocks['robActions_display_text_i2c'] = {
	...
	init : function() {
		...
		if (this.workspace.device === 'sensebox') {
			this.appendDummyInput().appendField(
					Blockly.Msg.ACTION_LCDI2C_SENSEBOX, 'ACTORTITEL')
					.appendField(dropDownPorts, 'ACTORPORT');
		} else {
			this.appendDummyInput().appendField(Blockly.Msg.ACTION_LCDI2C,
					'ACTORTITEL').appendField(dropDownPorts, 'ACTORPORT');
		}
		this.appendValueInput('OUT').setAlign(Blockly.ALIGN_RIGHT).appendField(
				Blockly.Msg.DISPLAY_SHOW + ' ' + Blockly.Msg.DISPLAY_TEXT);
		this.appendValueInput('COL').setCheck('Number').setAlign(
				Blockly.ALIGN_RIGHT).appendField(Blockly.Msg.DISPLAY_COL);
		this.appendValueInput('ROW').setCheck('Number').setAlign(
	...
	init : function() {
		...
		if (this.workspace.device == 'arduino') {
			...
			this.appendDummyInput().appendField(Blockly.Msg.DISPLAY_CLEAR)
					...
					.setAlign(Blockly.ALIGN_RIGHT).appendField(dropDownPorts,
							'ACTORPORT');
		} else {
			this.appendDummyInput().appendField(Blockly.Msg.DISPLAY_CLEAR);
		}
...
Blockly.Blocks['robActions_display_matrix'] = {
	...
	init : function() {
		...
		for (var i = 0; i < 8; i++) {
			...
			this.appendDummyInput().appendField(checkBoxes[(i * 8) + 0],
					'POINT' + (i * 8 + 0)).appendField(checkBoxes[(i * 8) + 1],
					'POINT' + (i * 8 + 1)).appendField(checkBoxes[(i * 8) + 2],
					'POINT' + (i * 8 + 2)).appendField(checkBoxes[(i * 8) + 3],
					'POINT' + (i * 8 + 3)).appendField(checkBoxes[(i * 8) + 4],
					'POINT' + (i * 8 + 4)).appendField(checkBoxes[(i * 8) + 5],
					'POINT' + (i * 8 + 5)).appendField(checkBoxes[(i * 8) + 6],
					'POINT' + (i * 8 + 6)).appendField(checkBoxes[(i * 8) + 7],
					'POINT' + (i * 8 + 7));
		}
		this.setPreviousStatement(true);
		this.setNextStatement(true);
...
Blockly.Blocks['robActions_play_tone'] = {
	...
	init : function() {
		...
		if (this.workspace.device === 'arduino'
			...
			this.appendValueInput('FREQUENCE').appendField(Blockly.Msg.PLAY)
					.appendField(dropDownPorts, 'ACTORPORT').appendField(
							Blockly.Msg.PLAY_FREQUENZ).setCheck('Number');
			...
			this.appendValueInput('FREQUENCE').appendField(Blockly.Msg.PLAY)
					.appendField(Blockly.Msg.PLAY_FREQUENZ).setCheck('Number');
		}
...
Blockly.Blocks['robActions_play_file'] = {
	...
	init : function() {
		...
		var file = new Blockly.FieldDropdown([ [ '1', '0' ], [ '2', '1' ],
				[ '3', '2' ], [ '4', '3' ], [ '5', '4' ] ]);
		this.appendDummyInput().appendField(
				Blockly.Msg.PLAY + ' ' + Blockly.Msg.PLAY_FILE).appendField(
				file, 'FILE');
		this.setPreviousStatement(true);
		this.setNextStatement(true);
...
Blockly.Blocks['robActions_play_setVolume'] = {
	...
	init : function() {
		...
		this.setColour(Blockly.CAT_ACTION_RGB);
		this.appendValueInput('VOLUME').appendField(
				Blockly.Msg.SET + ' ' + Blockly.Msg.PLAY_VOLUME + ' '
						+ Blockly.Msg.SENSOR_UNIT_PERCENT).setCheck('Number');
		this.setPreviousStatement(true);
		this.setNextStatement(true);
	...
	init : function() {
		...
		if (this.workspace.device === 'botnroll'
			...
			this.appendDummyInput().setAlign(Blockly.ALIGN_RIGHT).appendField(
					Blockly.Msg.BRICKLIGHT_COLOR).appendField(dropdownColor,
					'SWITCH_COLOR');
		}
		if (this.workspace.device === 'arduino'
				|| this.workspace.device === 'calliope'
			...
			this.appendDummyInput().appendField(Blockly.Msg.BRICKLIGHT)
					.setAlign(Blockly.ALIGN_RIGHT).appendField(Blockly.Msg.MOD)
					.appendField(dropdownLightState, 'SWITCH_BLINK');
		} else {
			this.appendDummyInput().setAlign(Blockly.ALIGN_RIGHT).appendField(
					Blockly.Msg.MOD).appendField(dropdownLightState,
					'SWITCH_BLINK');
		}
		this.setPreviousStatement(true);
		this.setNextStatement(true);
...
Blockly.Blocks['robActions_led_on'] = {
	...
	init : function() {
		...
		if (this.workspace.device === 'mbot') {
			ports = new Blockly.FieldDropdown([ [ Blockly.Msg.LEFT, '2' ],
					[ Blockly.Msg.RIGHT, '1' ] ]);
		}
...
Blockly.Blocks['robActions_sensorLight_on'] = {
	...
	init : function() {
		...
		var dropdownLightState = new Blockly.FieldDropdown([
				[ Blockly.Msg.ON, 'ON' ], [ Blockly.Msg.OFF, 'OFF' ] ]);
		this.appendDummyInput().appendField(Blockly.Msg.SET).appendField(
		...
		this.appendDummyInput().setAlign(Blockly.ALIGN_RIGHT).appendField(
				Blockly.Msg.MOD)
				.appendField(dropdownLightState, 'SWITCH_STATE');
		this.appendDummyInput().setAlign(Blockly.ALIGN_RIGHT).appendField(
...
Blockly.Blocks['robActions_brickLight_reset'] = {
	...
	init : function() {
		...
		this.appendDummyInput().appendField(Blockly.Msg.SENSOR_RESET)
				.appendField(Blockly.Msg.BRICKLIGHT).appendField(
...
Blockly.Blocks['robActions_set_relay'] = {
	...
	init : function() {
		this.setColour(Blockly.CAT_ACTION_RGB);
		var relayState = new Blockly.FieldDropdown([ [ Blockly.Msg.ON, 'OFF' ],
				[ Blockly.Msg.OFF, 'ON' ] ]);
		...
		if (this.workspace.device == 'arduino') {
			this.appendDummyInput().appendField(Blockly.Msg.SET_RELAY_ARDUINO)
					.appendField(dropDownPorts, 'ACTORPORT').appendField(
							relayState, 'RELAYSTATE');
		} else {
			this.appendDummyInput().appendField(Blockly.Msg.SET_RELAY)
					.appendField(dropDownPorts, 'ACTORPORT').appendField(
							relayState, 'RELAYSTATE');
		}
		this.setPreviousStatement(true);
...
Blockly.Blocks['robActions_serial_print'] = {
	...
	init : function() {
		this.setColour(Blockly.CAT_ACTION_RGB);
		this.appendValueInput('OUT').appendField(
				Blockly.Msg.ACTION_SERIAL_PRINT || "ACTION_SERIAL_PRINT")
...
Blockly.Blocks['robActions_write_pin'] = {
	init : function() {
		...
		var valueType = new Blockly.FieldDropdown([
				...
				[ Blockly.Msg.MODE_ANALOG, 'ANALOG' ] ], function(option) {
		...
		this.appendValueInput('VALUE').appendField(Blockly.Msg.PIN_WRITE)
				.appendField(valueType, 'MODE').appendField(
						Blockly.Msg.ACTION_IN).appendField(this.dropDownPorts,
						'ACTORPORT').setCheck('Number');
		this.setPreviousStatement(true);
		this.setNextStatement(true);
		this.setTooltip(function() {
			return Blockly.Msg['ACTOR_' + that.getFieldValue('MODE')
					+ 'IN_TOOLTIP']
					|| 'ACTOR_' + that.getFieldValue('MODE') + 'IN_TOOLTIP';
		});
	...
	updatePorts : function(option) {
		...
		this.dropDownPorts.arrow_
				&& this.dropDownPorts.arrow_.replaceChild(document
						.createTextNode(' '),
						this.dropDownPorts.arrow_.childNodes[0]);
		if (this.dropDownPorts.arrow_
				&& this.dropDownPorts.menuGenerator_.length > 1) {
			this.dropDownPorts.arrow_.replaceChild(document.createTextNode(' '
...
Blockly.Blocks['robActions_pin_set_pull'] = {
	...
	init : function() {
		...
		var pins = new Blockly.FieldDropdown(
				Blockly.Blocks.robConfigDefinitions.pinsDigital[this.workspace.subDevice]
						());
		this.appendDummyInput().appendField(
				Blockly.Msg.SET + ' ' + Blockly.Msg.PIN_PULL).appendField(
				Blockly.Msg.PIN_PULL_UP).appendField(
				Blockly.Msg.ON + ' ' + Blockly.Msg.SENSOR_PIN).appendField(
				pins, 'PIN_PORT'); // shouldnt be called PIN, would need a
...
Blockly.Blocks['robActions_sendData'] = {
	...
	init : function() {
		...
		this.INCREMENT = 1;
		this.DECREMENT = -1;
		this.appendDummyInput().appendField(Blockly.Msg.SEND_DATA_TO)
				.appendField(
						new Blockly.FieldDropdown([
								[ Blockly.Msg.SEND_DATA_SENSEMAP, 'SENSEMAP' ],
								[ Blockly.Msg.ACTION_SDCARD, 'SDCARD' ] ]),
						'DESTINATION');
		this.setPreviousStatement(true);
		this.setNextStatement(true);
	...
	getNames_ : function() {
		...
		if (names.length === 0) {
			names
					.push([
							Blockly.Msg.CONFIGURATION_NO_PHENOMENON
									|| Blockly
											.checkMsgKey('CONFIGURATION_NO_PHENOMENON'),
							'NO_PHENOMENON' ]);
		}
		return names;
	},
		...
		for (var i = 0; i < this.connectedSensorsCount; i++) {
			...
			if (names[dropDown.menuGenerator_.length]) {
				dropDown.menuGenerator_
						.push(names[dropDown.menuGenerator_.length]);
			}
		}
		if (names.length > 1) {
			dropDown.arrow_
	...
	domToMutation : function(xmlElement) {
		var connectedSensorsCount = parseInt(xmlElement.getAttribute('items'),
				10);
	...
	generateSensorInputs_ : function(numberOfSensorInputs) {
		...
		for (var i = 0; i < numberOfSensorInputs; i++) {
			this.appendSensorInput_(i, new Blockly.FieldDropdown(this
					.getNames_()));
		}
	},
	appendSensorInput_ : function(inputNumber, phenomenaDropdown) {
		this.appendValueInput('ADD' + inputNumber)
				.setAlign(Blockly.ALIGN_RIGHT).setCheck('Number').appendField(
						Blockly.Msg.BRICK_PHENOMENON).appendField(
						phenomenaDropdown, 'PHEN' + inputNumber);
	},
	updateShape_ : function(adjustment) {
		...
		case this.INCREMENT:
			...
			var phenomenaDropdown = new Blockly.FieldDropdown(this.getNames_())
			this.appendSensorInput_(this.connectedSensorsCount,
					phenomenaDropdown);
			this.connectedSensorsCount++;
			break;
		case this.DECREMENT:
			this.connectedSensorsCount--;
			var target = this.getInputTargetBlock('ADD'
					+ this.connectedSensorsCount);
			if (target) {
				target.unplug();
...
Blockly.Blocks['robActions_plot_point'] = {
	init : function() {
		...
		this.appendValueInput('VALUE').appendField(
				Blockly.Msg.ACTION_PLOT_POINT).appendField("on").appendField(
				dropDownPorts, 'ACTORPORT').appendField(
				Blockly.Msg.SENSOR_VALUE).setCheck('Number');
		this.appendValueInput('TICKMARK').appendField(
				Blockly.Msg.ACTION_PLOT_TICKMARK).setAlign(Blockly.ALIGN_RIGHT)
				.setCheck('Number');
		this.setPreviousStatement(true);
		this.setNextStatement(true);
...
Blockly.Blocks['robActions_plot_clear'] = {
	init : function() {
		...
		this.appendDummyInput().appendField(Blockly.Msg.ACTION_PLOT_CLEAR)
...
Blockly.Blocks['robActions_debug'] = {
	...
	init : function() {
		...
		this.setNextStatement(true);
		this
				.setTooltip("Debug any value or expression. The output is dependent on the system: simulation -> console; real systems -> write to serial or print");
	}
};
...
Blockly.Blocks['robActions_assert'] = {
	...
	init : function() {
		this.setColour("#646464");
		this.appendValueInput("OUT").appendField("assert").appendField(
				new Blockly.FieldTextInput(""), "TEXT").setCheck("Boolean");
		this.setPreviousStatement(true);
		this.setNextStatement(true);
		this
				.setTooltip("Assert an expression and optional supply a custom label for the assertion.");
	},
	onchange : function(e) {
